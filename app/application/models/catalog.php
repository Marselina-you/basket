  ...
  {	  
	  ...
	  { 
		...
		if($this->current_category["url"]=="catalog"){
			...
			$sql = $son->query("SELECT  p.id  FROM product p LEFT JOIN category c ON c.id=p.cat_id");
			$result = parent::query($sql);
			}
		else	
		{
			$son = new mysqli(HOST, USER, PASSWORD, NAME_BD);	
			$sql = $son->query("SELECT  p.id  FROM product p LEFT JOIN category c ON c.id=p.cat_id WHERE c.id=%d ").$filter;//запрос вернет обще кол-во продуктов в выбранной категории	
		...
		$sql = $son->query("SELECT  id  FROM product");//если категория не выбранна, то показать все товары каталога
		...
		if(empty($this->category_id)) {//если категория не выбрана то формируем запрос по всем имеющимся элементам
			$sql = $son->query("SELECT  *  FROM product ORDER BY id LIMIT %d , %d");
		...
		if(!empty($this->category_id))
			...
			if($this->current_category["url"]=="catalog"){
			$sql =  $son->query("SELECT  c.url as category_url, p.url as product_url, p.*  FROM product p LEFT JOIN category c ON c.id=p.cat_id  ORDER BY id LIMIT %d , %d");
			$result = parent::query($sql,$lower_bound,$step);
			}	
			else{
			$sql = $son->query("SELECT  c.url as category_url, p.url as product_url, p.*  FROM product p LEFT JOIN category c ON c.id=p.cat_id WHERE c.id=%d ".$filter." ORDER BY id LIMIT %d , %d");
